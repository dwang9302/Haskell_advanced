...
on the world.
 ...
 (IO action: allows for side effects)
...
(even though we didn't explicitly give them an argument).
 ...
 (doTwice plus1 :: Int -> Int can be thought of in two ways)
...
Note that the section notation can be translated to (more verbose) lambda
expression notation:

 (+2) is short for \x -> x + 2

 (3 *) is short for \x -> 3 * x

 Similarily, the section `(1:)`takes a list of numbers and returns a new list
 with `1` followed by input list. So
